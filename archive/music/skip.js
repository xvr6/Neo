...
module.exports = {
    ...
    aliases: ['next', 's'],
    category: "music",
    description: "Skips a song or a specific number of songs to skip. Note: skipping a specified number of songs includes skipping the current song. So the queue will jump from 1 to 5 if your input was 6.",
    usage: '[number of songs to skip]',
    run: async (client, message, args, queue, voice_channel) => { 
        if(cooldown.has(message.guild.id)){
            return message.reply({content: `To skip multiple songs, just input the number of songs you wish to skip.`}).then(msg => (setTimeout(() => msg.delete().catch(() => {}), 6000)));
        }
        ...
        let current = parseInt(server_queue.songNumb) 
            if(args[0]){
                var max = parseInt(server_queue.songs.length) - 1 
                    max = max - parseInt(server_queue.songNumb);
